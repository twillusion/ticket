...
def _status_for(result: ExtractResult) -> tuple[str, str]:
    n, errs = len(result.listings), len(result.errors)
    if not result.candidates:
        return "empty", ("no listing-like arrays found in any captured JSON. StubHub's page "
                         "structure may have changed, or listings didn't load. Run `probe`.")
    if n == 0:
        return "empty", f"found {len(result.candidates)} candidate arrays but 0 listings parsed ({errs} errors)"
    if errs:
        return "partial", f"{n} listings stored, {errs} parse issues"
    return "ok", f"{n} listings stored"
...
def run(sources: Sources, *, reparse_dir: Path | None = None, **capture_kwargs) -> RunOutcome:
    ...
    try:
        if reparse_dir:
            docs = load_raw(reparse_dir)
        else:
            docs = capture_event(sources.stubhub, raw_dir, **capture_kwargs).docs
        result = extract(docs, sources.stubhub.quantity, sources.stubhub.event_url)
        status, message = _status_for(result)
